...
async function main() {
  console.log("=== Trendyol Product Ranking Scraper ===");
  ...
  try {
    ...
    report.results.forEach((set) => {
      ...
      set.keywords.forEach((kw) => {
        console.log(
          `  "${kw.keyword}": ${
            kw.status === "Found"
              ? `Ranked at position ${kw.position} out of ${kw.totalProducts}`
    ...
    const filename = `trendyol_ranking_report_${timestamp}.json`;
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const filename = `trendyol_ranking_report_partial_${timestamp}.json`;
    await fs.writeFile(
      filename,
      JSON.stringify(
